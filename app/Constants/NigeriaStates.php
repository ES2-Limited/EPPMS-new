...
final class NigeriaStates
{
    ...
    {
        return collect(array_keys(self::LGAS))->mapWithKeys(fn (string $state): array => [$state => $state])->all();
    }
    ...
    {
        return collect(self::LGAS[$state] ?? [])->mapWithKeys(fn (string $lga): array => [$lga => $lga])->all();
    }
